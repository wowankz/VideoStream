...
rimraf(appDir, (err) => {
    ...
    ng = spawn('npm', ['run','build'], { shell: true, stdio: 'inherit', env: process.env });
...
const intervalId = setInterval(() => {
    ...
    fs.access(appDir, fs.constants.F_OK, (err) => {
        if (!err) {
            ...
            Wp = spawn('npx', ['webpack', '--config', `${webpackConfigChrome}/webpack.config.chrome.js`, '--color', `${true}`], { shell: true, stdio: 'inherit' });
